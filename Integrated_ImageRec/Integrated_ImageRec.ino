...
char model[] = "Integrated ImageRec";  // Change to your app name.
char versionStr[] = "1.0";
...
void customStringCallbackFromAndroid(String customString) {
  ...
  if (customString.equalsIgnoreCase("Perform ball test")) {
    ...
    ballcolorCommand[0] = sendBallColor(ballColor_1);
    ballcolorCommand[1] = sendBallColor(ballColor_2);
    ballcolorCommand[2] = sendBallColor(ballColor_3);
    lcd.print(ballcolorCommand);
    acc.write(ballcolorCommand,3);
  } else {
...
char sendBallColor(int ballColor) {
  switch (ballColor) {
    case BALL_NONE:
      return('N');
    case BALL_BLACK:
